...
void Diary::load_data(const string & load_path){    //load data
    if(load_path.substr(load_path.find('.')) != ".diary"){
        cout << "Wrong path or incorrect file extension." << endl;
        return;
    }
    ifstream load_file;
    load_file.open(load_path, ios::in );
    if(!load_file.is_open()){
        cout << "Fail to open file! No data has been read." << endl;
        return;
    }
    int record_num;
    ...
    load_file.close();
    cout << "Load data successfully. " << record_num << " records have been read."<< endl;
}
void Diary::write_data() const{ //write data
    if(diary_address.substr(diary_address.find('.')) != ".diary"){
        cout << "Wrong path or incorrect file extension." << endl;
        return;
    }
    ofstream store_file;
    ...
    if(!store_file.is_open()){
        cout << "Fail to open file! No data has been writen." << endl;
        return;
    }
...
void Diary::pdlist() const{
    ...
    cout << endl << "Executed successfully. List " << count << " entrys." << endl;
}
...
void Diary::pdshow(date show_date) const{
    if(diary_data.find(show_date) == diary_data.end()){
        cout << "No recond of" << show_date << '.' << endl;
        return;
    }
...
int Diary::pdremove(date remove_date){
    if(diary_data.find(remove_date) == diary_data.end()){
        cout << "No recond of" << remove_date << '.' << endl;
        return -1;
    }
